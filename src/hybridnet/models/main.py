...
class MeanTeacher:
    options = {
        ...
        'arch_var': 'baseline', # baseline, hybrid
        'ema_decay': 0.97,
...
def main(context):
    ...
    #tensorboard_writer.add_graph(model, (torch.autograd.Variable(torch.rand(1, 3, 32, 32)),))
    # Not working: Strided slide not supported
...
def train(train_loader, model, ema_model, optimizer, epoch, log, tensorboard_writer=None):
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        if args.logit_distance_cost >= 0:
            ...
            meters.update('res_loss', res_loss.data.item())
        else:
